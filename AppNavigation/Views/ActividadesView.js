...
export default class ActividadesView extends React.Component {
	constructor(props) {
		super(props);
	...
	async _getContenido() {
        const response = await APIHelper.get('admin/api/contenido/list_activities.php?id='+global.id);
        this.setState({
            isLoading: false,
    ...
    renderElement() {
    	if(this.state.dataSource){
        	...
        	return(
        		<Text style={Styles.noDataEndpoint}>No hay Actividades</Text>
        	)
        }
    }
				...
				<View style={Styles.container}>
					<View style={[Styles.colView, {height: '100%', width: '100%', flex: 1}]}>
						<View style={{height: 150}}>
	                        <ImageBackground source={require('../Assets/Images/fondo.jpg')} style={Styles.fondoDescripcionContenido}>
	                            <View style={Styles.viewDescripcionContenido}>
		                            <Text style={Styles.titleContenido}>ACTIVIDADES</Text>
		                            <Text style={Styles.subtitleContenido}>Conocé las Últimas Actividades del Club de Embajadores.</Text>
	                    ...
	                    <View style={[Styles.rowView, titleIntereses]}>
                            <View>
                                <Text style={{fontFamily: 'GothamBold'}}>Actividades según sus Intereses</Text>
                            </View>
                        </View>
                        { this.renderElement() }
					</View>
