...
//            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).'/fotos/',
			'upload_dir' => $_SERVER["DOCUMENT_ROOT"] . '/fotos/',
//            'upload_url' => $this->get_full_url().'/fotos/',
			'upload_url' => $_SERVER["DOCUMENT_ROOT"] . '/fotos/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => $_GET['nombre_input'],
        ...
        public function post($print_response = true) {
        	...
        	if ($upload && is_array($upload['tmp_name'])) {
        		...
        		foreach ($upload['tmp_name'] as $index => $value) {
        			...
        			// TODO lo guardo en session
        			$file_content = file_get_contents($upload['tmp_name'][$index]);

        			$foto = new FileUpload();
        			$foto->name = $file_name ? $file_name : $upload['name'][$index];
        			$foto->size = $size ? $size : $upload['size'][$index];
        			$foto->type = $upload['type'][$index];
        			$foto->content = $file_content;
					
					$ext = explode(".", $foto->name);
					$foto->namecode = time() . "." . $ext[count($ext) - 1];

					
        			//$_SESSION['foto'] = 'holaaa';//$foto;
					$_SESSION[$_GET['nombre_input']] = $foto;
        			
//        			$file_content = file_get_contents($upload['tmp_name'][$index]);
//        			$_SESSION['foto-content'] = $file_content;
        		}
        	} else {
        		...
        		$foto = new FileUpload();
        		$foto->name = $file_name ? $file_name : (isset($upload['name']) ? $upload['name'] : null);
        		$foto->size = $size ? $size : (isset($upload['size']) ? $upload['size'] : $this->get_server_var('CONTENT_LENGTH'));
        		$foto->type = isset($upload['type']) ? $upload['type'] : $this->get_server_var('CONTENT_TYPE');
        		$foto->content = $file_content;
